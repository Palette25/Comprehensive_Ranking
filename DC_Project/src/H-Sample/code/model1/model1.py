...
print('End Reading Operation Datas~')
...
print('End Reading Transaction Datas~')
...
label_test = pd.read_csv(path + 'sample.csv').drop('Tag', axis = 1)
label_train = pd.read_csv(path + 'tag_train_new.csv')
...
lgb_pred.to_csv(path + 'model1_res.csv', index=False, header=True, sep=',', columns=['UID','Tag'])

print('LightGBM Model, Done~')
